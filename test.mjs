...
export function place(db: DatabaseSync): void { save(db); }`,
  ...
  check("--gate-json verdict declares spec 0.20", v?.spec === "0.20", JSON.stringify(v)?.slice(0, 120));
...
// ── 3f. diff/gains disclose a producing-build mismatch (§2.1 — baseline-invalidation) ──────────────
{
  ...
  fs.writeFileSync(path.join(d, "cur.json"), JSON.stringify({ candor: { version: "bbbbbbb", spec: "0.20" },
    functions: [{ fn: "a.leaf", inferred: ["Net", "Log"], direct: ["Net", "Log"] }] }));
  fs.writeFileSync(path.join(d, "base.json"), JSON.stringify({ candor: { version: "aaaaaaa", spec: "0.20" },
  ...
  // same-build → no note
  fs.writeFileSync(path.join(d, "base.json"), JSON.stringify({ candor: { version: "bbbbbbb", spec: "0.20" },
    functions: [{ fn: "a.leaf", inferred: ["Net"], direct: ["Net"] }] }));
...
// EXACT exit code (1 vs 2 is load-bearing: violation vs could-not-evaluate).
{
  ...
  const rep = (fns) => JSON.stringify({ candor: { version: "ttttttt", spec: "0.20" }, functions: fns });
  ...
  fs.writeFileSync(path.join(d, "oldbase.json"), JSON.stringify({ candor: { version: "aaaaaaa", spec: "0.20" },
    functions: [{ fn: "app.db.save", inferred: ["Db"], direct: ["Db"] }] }));
  fs.writeFileSync(path.join(d, "cur2.json"), JSON.stringify({ candor: { version: "bbbbbbb", spec: "0.20" },
  ...
  fs.writeFileSync(path.join(d, "samebase.json"), JSON.stringify({ candor: { version: "bbbbbbb", spec: "0.20" },
    functions: [{ fn: "app.db.save", inferred: ["Db"], direct: ["Db"] }] }));
  ...
  fs.writeFileSync(path.join(d, "obase.json"), JSON.stringify({ candor: { version: "ccccccc", spec: "0.20" },
    functions: [{ fn: "m.g", inferred: ["Fs"], direct: ["Fs"] }] }));
  fs.writeFileSync(path.join(d, "obase.callgraph.json"), JSON.stringify({ "m.f": ["m.g"], "m.g": [] }));
  fs.writeFileSync(path.join(d, "ocur.json"), JSON.stringify({ candor: { version: "ccccccc", spec: "0.20" },
  ...
  fs.writeFileSync(path.join(d, "covcur.json"), JSON.stringify({ candor: { version: "ddddddd", spec: "0.20" },
    functions: [{ fn: "m.f", inferred: ["Net"], direct: ["Net"] }],
    coverage: { uncovered: [{ name: "blinddep", calls: 2 }] } }));
  fs.writeFileSync(path.join(d, "covbase.json"), JSON.stringify({ candor: { version: "ddddddd", spec: "0.20" },
    functions: [] }));
...
export { Settings };`,
  ...
  fs.writeFileSync(`${munk}/.candor/report.json`, JSON.stringify({ candor: { version: "t", toolchain: "node", spec: "0.20" },
  ...
  fs.writeFileSync(`${clean}/.candor/report.json`, JSON.stringify({ candor: { version: "t", toolchain: "node", spec: "0.20" },
...
// exercised argless in CLI-10.
{
  ...
  fs.writeFileSync(path.join(d, "r.json"), JSON.stringify({ candor: { version: "ttttttt", spec: "0.20" },
...
// ("spec 0.20", no stale generation strings — AGENTS.md shipped "spec 0.7" examples a full generation
...
// the reference — the family ruling the baseline/pure semantics cite).
{
  for (const f of ["README.md", "AGENTS.md"]) {
    ...
    check(`${f} states the current spec contract (spec 0.20)`, doc.includes("spec 0.20"));
